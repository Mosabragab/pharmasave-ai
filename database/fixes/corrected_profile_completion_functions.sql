-- 🚨 CORRECTED Profile Completion Functions
-- This file fixes the schema mismatch that was causing incorrect completion calculations
-- 
-- Issues Fixed:
-- 1. Functions now reference ACTUAL database columns (not non-existent ones)
-- 2. Progressive access logic implemented (educational content at 30%+)
-- 3. Demo marketplace access added (50%+)
-- 4. Verification submission logic corrected
-- 5. Triggers properly update completion percentages
...
BEGIN
    ...
    -- Ensure we don't exceed 100% before verification
...
-- Function to update profile completion flags (CORRECTED & ENHANCED)
...
DECLARE
    ...
    can_access_full_marketplace BOOLEAN := false;
...
BEGIN
    ...
    UPDATE pharmacies SET
        ...
        marketplace_access = can_access_full_marketplace,
        updated_at = NOW()
    ...
    FOR pharmacist_record IN SELECT * FROM pharmacists WHERE pharmacy_id = pharmacy_id LOOP
        ...
        UPDATE pharmacists SET
            ...
            -- PROGRESSIVE FEATURE ACCESS (NEW!)
            can_create_listings = can_access_full_marketplace,
            can_approve_transactions = can_access_full_marketplace,
            updated_at = NOW()
...
-- NEW: Function to get progressive access status for a pharmacy
...
RETURNS TABLE(
    ...
    can_access_full_marketplace BOOLEAN,
...
BEGIN
    ...
    can_access_full_marketplace := (overall_percent >= 70);
    ...
    ELSIF overall_percent < 70 THEN
        next_milestone := 'Full Marketplace Access';
